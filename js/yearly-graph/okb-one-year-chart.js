...
var xmlhttp = new XMLHttpRequest();
  ...
  var url = "https://min-api.cryptocompare.com/data/pricemulti?fsyms=BTC,okb,EOS,BCH,LTC,XRP,OKB&tsyms=USD";
  var urlHistoricalBtc = "https://min-api.cryptocompare.com/data/histominute?fsym=OKB&tsym=USD&limit=1";
...
function parseJsonFromCryptoCompare(json) {
  ...
  var okbereum = json["OKB"]["USD"];

  document.getElementById("okbLast").innerHTML = okbereum;
  document.getElementById("lastUpdated").innerHTML = time;
                
  colorText("okbLast",urlHistoricalBtc);
...
(function getData() {
  var url = "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=OKB&tsyms=USD";
  url = encodeURI(url);
  console.log(url);

  $.getJSON(url,function(data) {

      var hokb_crypto_data = '';
      var lokb_crypto_data = '';
      var pcokb_crypto_data = '';
      var mcokb_crypto_data = '';

      $.each(data.RAW, function(key, value){

        lokb_crypto_data += value.USD.LOWDAY;
        hokb_crypto_data += value.USD.HIGHDAY;
        pcokb_crypto_data += value.USD.CHANGEPCT24HOUR;
        mcokb_crypto_data += value.USD.MKTCAP;

      });

      $('#okbHigh').html(hokb_crypto_data);
      $('#okbLow').html(lokb_crypto_data);
      $('#okbPct').html(pcokb_crypto_data);
      $('#okbMkc').html(mcokb_crypto_data);
