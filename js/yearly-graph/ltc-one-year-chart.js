...
var xmlhttp = new XMLHttpRequest();
  ...
  var url = "https://min-api.cryptocompare.com/data/pricemulti?fsyms=BTC,ltc,EOS,BCH,LTC,XRP&tsyms=USD";
  var urlHistoricalBtc = "https://min-api.cryptocompare.com/data/histominute?fsym=LTC&tsym=USD&limit=1";
...
function parseJsonFromCryptoCompare(json) {
  ...
  var ltcereum = json["LTC"]["USD"];

  document.getElementById("ltcLast").innerHTML = ltcereum;
  document.getElementById("lastUpdated").innerHTML = time;
                
  colorText("ltcLast",urlHistoricalBtc);
...
(function getData() {
  var url = "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=LTC&tsyms=USD";
  url = encodeURI(url);
  console.log(url);

  $.getJSON(url,function(data) {

      var hltc_crypto_data = '';
      var lltc_crypto_data = '';
      var pcltc_crypto_data = '';
      var mcltc_crypto_data = '';

      $.each(data.RAW, function(key, value){

        lltc_crypto_data += value.USD.LOWDAY;
        hltc_crypto_data += value.USD.HIGHDAY;
        pcltc_crypto_data += value.USD.CHANGEPCT24HOUR;
        mcltc_crypto_data += value.USD.MKTCAP;

      });

      $('#ltcHigh').html(hltc_crypto_data);
      $('#ltcLow').html(lltc_crypto_data);
      $('#ltcPct').html(pcltc_crypto_data);
      $('#ltcMkc').html(mcltc_crypto_data);
